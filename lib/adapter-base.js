...
(function () {
  class LLMAdapter {
    ...
    insertIntoInput(text) {
      ...
      // Contenteditable: a single, trusted strategy.
      //
      // Lexical, ProseMirror, Slate, and Draft all observe paste events
      // with a DataTransfer payload — that's the one universal hook.
      // We don't verify and don't cascade: the previous cascade caused
      // double-insertion when an earlier strategy succeeded but the
      // post-hoc textContent check missed nested DOM (Lexical wraps
      // inserted text in spans, so it doesn't appear at the top level
      // synchronously).
      inp.focus();
      try {
        // Position the cursor at the end so the paste appends rather
        // than overwriting any current selection.
        ...
        navigator.clipboard.writeText(text).catch(() => {});
        alert(
          "PolyPrompt: prompt copied to clipboard. Paste with Cmd/Ctrl+V.",
        );
      }
    }
  }
