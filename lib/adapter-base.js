...
(function () {
  class LLMAdapter {
    ...
    insertIntoInput(text) {
      ...
      if (!inp) {
        alert(
          "PolyPrompt: couldn't find the chat input on this page.\n\n" +
            "The prompt has been copied to your clipboard — paste it manually.",
        );
      ...
      // Strategy 1: plain <textarea> / <input>.
      ...
      // Contenteditable. Try several strategies — different rich-text
      // frameworks (ProseMirror, Lexical, Slate, Draft) respond to
      // different events.

      // Strategy 2: classic execCommand (still works for many editors,
      // including ChatGPT's, but Lexical ignores it).
      try {
        const ok = document.execCommand("insertText", false, text);
        if (ok && inp.textContent.includes(text.slice(0, 20))) return;
      } catch (e) {
        /* fall through */
      }

      // Strategy 3: simulate paste with a DataTransfer. This is what
      // Lexical and ProseMirror listen to most reliably.
      try {
        ...
        if (inp.textContent.includes(text.slice(0, 20))) return;
      } catch (e) {
        /* fall through */
      }

      // Strategy 4: InputEvent with insertFromPaste.
      try {
        inp.dispatchEvent(
      ...
      // Strategy 5: last resort — copy to clipboard and tell the user.
      navigator.clipboard.writeText(text).catch(() => {});
      alert(
        "PolyPrompt couldn't auto-insert on this site.\n\n" +
          "The prompt has been copied to your clipboard — paste it with Cmd/Ctrl+V.",
      );
    }
  }
