...
(function () {
  class LLMAdapter {
    ...
    async init() {
      // Wait for the site's main UI to render before we start touching DOM.
      await this.waitForElement(this.selectors.sidebar, 15000);
      this.injectPanel();
      this.watchChatList();
    }
    ...
    insertIntoInput(text) {
      const inp = document.querySelector(this.selectors.input);
      if (!inp) {
        alert("PolyPrompt: couldn't find the chat input on this page.");
        return;
      }
