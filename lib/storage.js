...
(function () {
  ...
  // Chrome invalidates the extension context when the extension is
  // reloaded while content scripts are still attached to open pages.
  // Subsequent chrome.* calls throw "Extension context invalidated."
  // This is a Chrome quirk, not a bug in the page or the storage layer,
  // and the only fix is for the user to refresh the affected tab.
  // We catch the error so we don't pollute the page console with a
  // scary red uncaught-promise.
  let warned = false;
  function handleContextError(e) {
    ...
    if (msg.includes("Extension context invalidated") ||
      ...
      if (!warned) {
        console.info(
          "[PolyPrompt] Extension was reloaded — refresh this tab to reconnect.",
        );
        warned = true;
      }
      return true;
  ...
  async function readAll() {
    try {
      ...
      if (handleContextError(e)) return empty();
      throw e;
    }
  }
